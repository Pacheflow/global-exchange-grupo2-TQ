...
class ProveedorTasasHTTP:
    ...
    def obtener(self, moneda_base, monedas_cotizadas):
        ...
        if not isinstance(payload, dict):
            raise ProveedorTasasError(
                "La respuesta del proveedor tiene un formato inválido."
            )
        if payload.get("result") not in (None, "success"):
            raise ProveedorTasasError(
                "El proveedor informó que la consulta no fue exitosa."
            )

        base_respuesta = str(
            payload.get("base_code") or payload.get("base") or ""
        ).upper()
        ...
        for codigo in codigos:
            try:
                ...
                raise ProveedorTasasError(
                    f"La tasa de {codigo} no es numérica."
                ) from exc
            if not valor.is_finite() or valor <= 0 or not math.isfinite(float(valor)):
                raise ProveedorTasasError(
                    f"La tasa de {codigo} debe ser positiva y finita."
                )
        ...
        try:
            ...
            raise ProveedorTasasError(
                "La fecha/hora del proveedor es inválida."
            ) from exc
