...
class ConsultaProveedorTasas(models.Model):
    """Respuesta externa válida conservada para trazabilidad y fallback."""
    ...
    def __str__(self):
        return (
            f"{self.fuente} · {self.moneda_base.codigo} · "
...
class TasaReferencia(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=~Q(moneda_base=F("moneda_cotizada")),
    ...
    def __str__(self):
        return (
            f"{self.moneda_base.codigo}/{self.moneda_cotizada.codigo}: "
            f"{self.valor}"
        )
