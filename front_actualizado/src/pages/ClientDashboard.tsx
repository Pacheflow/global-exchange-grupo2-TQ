...
import TasasView from '../components/crud/TasasView';
...
const sidebarItems = [
  ...
  { icon: '⇄', label: 'Operaciones', href: '/dashboard/operaciones' },
  { icon: '↕', label: 'Transacciones', href: '/dashboard/transacciones' },
  { icon: '📄', label: 'Facturas', href: '/dashboard/facturas' },
  { icon: '🔔', label: 'Notificaciones', href: '/dashboard/notificaciones', badge: 2 },
  { icon: '🏢', label: 'Mi cliente', href: '/dashboard/cliente' },
  { icon: '⚙', label: 'Configuración', href: '/dashboard/config' },
];
...
function StatCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="ge-card" style={{ padding: '22px 24px' }}>
      ...
      {sub && <div style={{ marginTop: 6, fontSize: 12, color: 'var(--text-3)', fontFamily: 'DM Sans' }}>{sub}</div>}
    </div>
  );
}
...
export default function ClientDashboard() {
  ...
  const currentTab = pathParts[1] || 'resumen';

  const setActiveTab = (t: string) => {
    navigate(t === 'resumen' ? '/dashboard' : `/dashboard/${t}`);
  };
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <main style={{ flex: 1, padding: '80px 0 0', overflowX: 'hidden' }}>
          ...
          <div style={{ padding: '24px 40px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            ...
            {/* Tabs */}
            <div style={{ padding: '0 40px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {(['resumen', 'operaciones', 'transacciones', 'pagos', 'tasas'] as const).map(t => (
                <button
                  key={t}
            ...
            {activeTab === 'resumen' && (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
                  <StatCard label="OPERACIONES (MES)" value="12" sub="↑ 3 más que el mes anterior" />
                  <StatCard label="ÚLTIMO MONTO USD" value="2.500" sub="Venta · 30 ago" />
                  <StatCard label="TRANSACCIONES" value="8" sub="COMPLETADAS este mes" color="var(--success)" />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  ...
                  <div className="ge-card" style={{ padding: 24 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      {[
                        { icon: '↓', label: 'Comprar divisas', color: 'var(--success)' },
                        { icon: '↑', label: 'Vender divisas', color: 'var(--danger)' },
                        { icon: '⇄', label: 'Convertir', color: 'var(--primary)' },
                        { icon: '📋', label: 'Ver historial', color: 'var(--text-2)' },
                      ].map(a => (
                        <button
                          key={a.label}
              ...
              <div style={{ maxWidth: 640 }}>
                ...
                <div className="ge-card" style={{ padding: 28 }}>
                  ...
                  <form onSubmit={handleBuySubmit}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 16, marginBottom: 16, alignItems: 'end' }}>
                      <div>
                      ...
                      <div style={{ marginBottom: 4 }}>
                        <button type="button" onClick={() => {
                          ...
                          setBuyFrom(buyTo);
                          setBuyTo(temp);
                        }} className="ge-btn-outline" style={{ padding: '8px 12px', borderRadius: '50%' }}>
                          ⇄
                        </button>
                      </div>
                      <div>
                    ...
                    {buyAmount && parseFloat(buyAmount) > 0 && (
                      <div style={{ background: 'var(--surface-2)', borderRadius: 10, padding: '16px 20px', marginBottom: 20, border: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-3)', letterSpacing: '0.08em', fontFamily: 'DM Sans' }}>
                            RESUMEN DE OPERACIÓN
                          </div>
                          <div style={{ fontSize: 10, color: 'var(--primary)', background: 'var(--primary-muted)', padding: '2px 6px', borderRadius: 4, fontFamily: 'DM Sans', fontWeight: 600 }}>
                            SIMULACIÓN
                          </div>
                        </div>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--text-3)', fontFamily: 'DM Sans', borderTop: '1px solid var(--border-subtle)', paddingTop: 12 }}>
                          Última actualización de tasas: Hace 2 minutos. Los valores mostrados son aproximados y pueden variar al confirmar la operación.
                        </div>
                      </div>
                    )}

                    <button type="submit" className="ge-btn-primary" style={{ width: '100%', padding: '13px', fontSize: 15, justifyContent: 'center' }}>
                      Continuar →
                    </button>
                  </form>
                </div>
                    ...
                    <tbody>
                      {transactions.map((t, i) => (
                        <tr key={t.id} className="table-row-hover" style={{ borderBottom: i < transactions.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                          ...
                          <td style={{ padding: '14px 16px' }}>
                            <button className="ge-btn-ghost" style={{ fontSize: 12, padding: '4px 10px' }}>
                              Ver →
                            </button>
                          </td>
                        </tr>
